...
function handleOpenProjectCommand(command) {
    ...
    var shouldReportDiagnostics = getEnvironmentVariable("SEMMLE_TYPESCRIPT_REPORT_DIAGNOSTICS", function (v) { return v.trim().toLowerCase() === "true"; }, false);
    ...
    function getStem(file) {
        ...
        if (file.endsWith(".d.mts") || file.endsWith(".d.cts")) {
            var base_1 = pathlib.basename(file);
            return base_1.substring(0, base_1.length - '.d.mts'.length);
        }
...
function reset() {
    state = new State();
    state.typeTable.restrictedExpansion = getEnvironmentVariable("SEMMLE_TYPESCRIPT_NO_EXPANSION", function (v) { return v.trim().toLowerCase() === "true"; }, true);
    state.typeTable.skipExtractingTypes = getEnvironmentVariable("CODEQL_EXTRACTOR_JAVASCRIPT_OPTION_SKIP_TYPES", function (v) { return v.trim().toLowerCase() === "true"; }, false);
}
...
if (process.argv.length > 2) {
    ...
    if (argument === "--version") {
        ...
        for (var _i = 0, _a = state.project.program.getSourceFiles(); _i < _a.length; _i++) {
            var sf = _a[_i];
            if (/lib\..*\.d\.ts/.test(pathlib.basename(sf.fileName)) || pathlib.basename(sf.fileName) === "lib.d.ts")
                continue;
            handleParseCommand({
                command: "parse",
