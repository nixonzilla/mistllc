...
var TypeTable = (function () {
    function TypeTable() {
        ...
        this.skipExtractingTypes = false;
    ...
    TypeTable.prototype.getId = function (type, unfoldAlias) {
        var _a;
        if (this.skipExtractingTypes)
            return null;
        var cached = (_a = this.idCache.get(type)) !== null && _a !== void 0 ? _a : [undefined, undefined];
        var cachedValue = cached[unfoldAlias ? 1 : 0];
        if (cachedValue !== undefined)
            return cachedValue;
        var result = this.getIdRaw(type, unfoldAlias);
        cached[unfoldAlias ? 1 : 0] = result;
        return result;
    };
    TypeTable.prototype.getIdRaw = function (type, unfoldAlias) {
    ...
    TypeTable.prototype.getTypeString = function (type, unfoldAlias) {
        ...
        if (symbol != null) {
            ...
            if (flags & ts.TypeFlags.TypeVariable) {
                ...
                else if (symbol.parent == null || isFunctionTypeOrTypeAlias((_a = symbol.declarations) === null || _a === void 0 ? void 0 : _a[0])) {
    ...
    TypeTable.prototype.getSymbolId = function (symbol) {
        if (symbol.flags & ts.SymbolFlags.Alias) {
            var aliasedSymbol = this.typeChecker.getAliasedSymbol(symbol);
            if (aliasedSymbol.$id !== -1) {
    ...
    TypeTable.prototype.makeDeduplicatedTypeStringVector = function (tag, types, length) {
        if (length === void 0) { length = types.length; }
        var seenIds = new Set();
        var numberOfSeenIds = 0;
        var hash = tag;
        for (var i = 0; i < length; ++i) {
            var id = this.getId(types[i], false);
            if (id == null)
    ...
    TypeTable.prototype.getSignatureString = function (kind, signature) {
        var modifiers = signature.getDeclaration() ? ts.getModifiers(signature.getDeclaration()) : [];
    ...
    TypeTable.prototype.extractSymbolBaseTypes = function (symbol, symbolId) {
        ...
        for (var _i = 0, _c = symbol.declarations || []; _i < _c.length; _i++) {
            ...
            if (ts.isClassLike(decl) || ts.isInterfaceDeclaration(decl)) {
                for (var _d = 0, _e = decl.heritageClauses || []; _d < _e.length; _d++) {
                    var heritage = _e[_d];
                    for (var _f = 0, _g = heritage.types; _f < _g.length; _f++) {
                        var typeExpr = _g[_f];
                        var superType = this.typeChecker.getTypeFromTypeNode(typeExpr);
                        if (superType == null)
                            continue;
                        var baseTypeSymbol = superType.symbol;
                        baseTypeSymbol = (_b = (_a = baseTypeSymbol === null || baseTypeSymbol === void 0 ? void 0 : baseTypeSymbol.type) === null || _a === void 0 ? void 0 : _a.symbol) !== null && _b !== void 0 ? _b : baseTypeSymbol;
    ...
    TypeTable.prototype.checkExpansiveness = function (type) {
        ...
        function search(type, expansionDepth) {
            ...
            for (var _i = 0, _a = type.getProperties(); _i < _a.length; _i++) {
                var symbol = _a[_i];
                var propertyType = typeTable.tryGetTypeOfSymbol(symbol);
                if (propertyType == null)
                    continue;
...
function isFunctionTypeOrTypeAlias(declaration) {
    if (declaration == null)
        return false;
    return declaration.kind === ts.SyntaxKind.FunctionType || declaration.kind === ts.SyntaxKind.TypeAliasDeclaration;
}
function isTypeSelfReference(type) {
    var _a, _b, _c;
    if (type.aliasSymbol != null) {
        var aliasTypeArguments = type.aliasTypeArguments;
        if (aliasTypeArguments == null)
            return true;
        var declaration = (_a = type.aliasSymbol.declarations) === null || _a === void 0 ? void 0 : _a[0];
        if (declaration == null || declaration.kind !== ts.SyntaxKind.TypeAliasDeclaration)
            return false;
        var alias = declaration;
        for (var i = 0; i < aliasTypeArguments.length; ++i) {
            if (((_c = (_b = aliasTypeArguments[i].symbol) === null || _b === void 0 ? void 0 : _b.declarations) === null || _c === void 0 ? void 0 : _c[0]) !== alias.typeParameters[i]) {
                return false;
            }
        }
        return true;
